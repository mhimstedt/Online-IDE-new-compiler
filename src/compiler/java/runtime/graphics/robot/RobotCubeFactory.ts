...
export class RobotCubeFactory {
    ...
    async init() {
        ...
        for(let i = 0; i < this.farben.length; i++){
            this.farbeToBrickMaterialMap.set(this.farben[i], new THREE.MeshLambertMaterial({
                map: this.world3d.textureManager3d.getSpritesheetBasedTexture("robot", 4 + i, this.world3d.renderer),
    ...
    getGrassCube() {
        ...
        const material = new THREE.MeshLambertMaterial({
            map: this.world3d.textureManager3d.getSpritesheetBasedTexture("robot", 0, this.world3d.renderer),
            side: THREE.DoubleSide
        });
    ...
    public initNorthArrow() {
        ...
        const material = new THREE.MeshBasicMaterial({ 
            map: this.world3d.textureManager3d.getSpritesheetBasedTexture("robot", 11, this.world3d.renderer),
            side: THREE.DoubleSide,
            transparent: true
        });
